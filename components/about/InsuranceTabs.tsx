    ...
    <section className="max-w-[77rem] mx-auto px-4 py-12">
      ...
      <div className="flex flex-wrap items-center justify-between mb-6 gap-4">
        ...
        <div className="flex bg-[#F6F8FA] p-1 rounded-full shadow-sm">
          {(['Illinois'] as const).map((state) => (
            <button
              key={state}
              onClick={() => setActiveTab(state)}
              className={`lg:px-28 px-11 lg:py-3 py-2 rounded-full font-medium text-xl cursor-pointer transition ${activeTab === state ? 'bg-[#88C240] text-white' : 'text-gray-500'
                }`}
            >
              {state}
            </button>
